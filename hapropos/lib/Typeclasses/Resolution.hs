...
literalsAreComplementary p q = p == Not q || q == Not p -- need to check if this works

hasComplementIn :: Literal -> Literal -> Bool
hasComplementIn p q = p == Not q || q == Not p
...
--
  ...
  complimentaryClause :: P.Prop -> P.Prop
  complimentaryClause (P.Not p) = p
  complimentaryClause p = Not p

  {-
  resolution c1 c2 =
      let complements = S.filter (\x -> S.member (complement x) c2) c1
      in if S.null complements
        then Left NoncomplementaryLiterals
        else Right (S.union (S.difference c1 complements) (S.difference c2 (S.map complement complements)))
  -}

-- clausePairs = filter (\(a, b) -> literalsAreComplementary a b) [(a, b)| a <- (S.toList knowledgeBase), b <- (S.toList knowledgeBase)] -- all possible pairs of clauses that contain complementary literals.
